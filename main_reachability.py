...
if __name__ == "__main__":
    ...
    for graph_rad in tqdm.tqdm(conf["graph_edge_radii"]):
        ...
        for repeat in range(int(conf["num_simulation_repeats"])):
            ...
            (
                ...
                average_degree,
            ) = x.infect_till_saturation(modality="irreversable", verbose=False)
            ...
            try:
                ...
                residuals_dict[graph_rad].extend(
                    fraction_infected_list - logistic_curve_data
                )
    ...
    residuals_dict = {
        k: v for k, v in list(zip(average_degrees, residuals_dict.values()))
    }
